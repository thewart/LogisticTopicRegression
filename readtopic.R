path <- "/home/seth/analysis/logtopreg/fit37_nosex/"
...
ggplot(merge(etasumm[topic=="S3"],Xdf,by="FocalID"),aes(y=prob,x=age,color=sex)) + 
...
#topic contents
topic <- data.table(behav=names(Y)[-(1:3)], #see bottom for renamed behaviors
...
topicsummary <- topic[iter > 100 & topic %in% nzmu$topic,
                      .(value=mean(value)),by=.(topic,type,behav)] %>% .[value > 1.1,.(topic,type,value=(value-1)/max(value-1)),by=behav]
...
ggplot(topicsummary[topic %in% c("S3")],aes(y=value,x=topic,fill=type,label=behav)) + geom_point() +
  geom_label_repel(fontface="bold",size=4,force=3) + coord_cartesian(ylim = c(0,1)) + theme_classic() + 
  scale_fill_brewer(drop=F,palette = "Accent", guide=guide_legend(title="")) + 
